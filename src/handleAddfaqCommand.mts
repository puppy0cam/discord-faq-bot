...
import { callbackAddfaq } from "./callbackAddfaq.mjs";
import { createJsonResponse } from "./createJsonResponse.mjs";
import type { Env } from "./env.mjs";
import { getCommandParameter } from "./getCommandParameter.mjs";

export function handleAddfaqCommand(interaction: APIChatInputApplicationCommandInteraction, env: Env, context: ExecutionContext): Response {
    const question = getCommandParameter(interaction, "question");
    const answer = getCommandParameter(interaction, "answer");
    if (question?.type !== 3 || answer?.type !== 3) {
        return createJsonResponse({
            type: 4,
            data: {
                content: "You must provide a question and an answer",
                flags: 4,
            },
        });
    }
    ...
    context.waitUntil(callbackAddfaq(interaction, env, question, answer));
